...
You will have to add at least 4 additional actions to the model: OpenObject, CloseObject, PickupObject, and PlaceHeldObject (for PlaceHeldObject syntax, check the update below). Each of these is supported by the Thor controller, and you can find the syntax for each [here](https://ai2thor.allenai.org/tutorials/actions). 

__Update (as of 3/11): Extra credit does not require a different executable. If you've already replaced the original build, we recommend re-downloading the original (you can just copy the wget command from setup.sh), and using that again. In order to place the tomato in the microwave, you can use PlaceHeldObject (not PutObject). This requires the agent to be holding the tomato, and takes the id of the microwave in the "objectId" field, so the action dict would look like {"action":"PlaceHeldObject", "objectId":microwave_id}.__
